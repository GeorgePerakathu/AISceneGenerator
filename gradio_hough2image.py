...
def process(input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, value_threshold, distance_threshold):
    try:
        # Print debug info
        print(f"Input image shape: {input_image.shape if input_image is not None else None}")
...
with block:
    ...
    inputs = [
        ...
        ddim_steps, guess_mode, strength, scale, seed, 
        eta, value_threshold, distance_threshold
    ]
    
    # Connect the run button to the process function
    run_button.click(
        fn=process,
        inputs=inputs,
        outputs=gallery,
